...
const HELP = `agent-board - SQLite-backed Jira-like CLI and MCP server

Usage:
  agent-board [--db <file>] schema
  agent-board [--db <file>] list <entity> [--limit <n>] [--field <value> ...]
  agent-board [--db <file>] get <entity> <uuid>
  agent-board [--db <file>] create <entity> --field <value> ...
  agent-board [--db <file>] update <entity> <uuid> --field <value> ...
  agent-board [--db <file>] delete <entity> <uuid>
  agent-board [--db <file>] mcp
  agent-board [--db <file>] mcp-http [--host <host>] [--port <port>] [--path <path>]
              [--allowed-origin <origin[,origin...]>]

All entities in db/schema.sql are available.

Values accept strings, null, true, false, or JSON prefixed with json:.
The database defaults to db/agent-board.sqlite and can also be set
with AGENT_BOARD_DB. --data and AGENT_BOARD_DATA remain database-path aliases.
MCP mode speaks newline-delimited JSON-RPC over stdio.
...
async function runCli(argv, io = {}) {
  ...
  const configuredDatabase = options.db || options.data || process.env.AGENT_BOARD_DB || process.env.AGENT_BOARD_DATA;
  const databaseFile = configuredDatabase || path.join(process.cwd(), 'db', 'agent-board.sqlite');
  const shouldMigrateDefault = !configuredDatabase;
  delete options.db;
  delete options.data;
  const databaseExisted = databaseFile !== ':memory:' && fs.existsSync(databaseFile);
  const store = new SqliteStore(databaseFile);
  if (shouldMigrateDefault && !databaseExisted) {
    const legacyFile = process.env.AGENT_BOARD_LEGACY_DATA || path.join(process.cwd(), '.agent-board', 'data.json');
    if (fs.existsSync(legacyFile)) {
      const legacy = JSON.parse(fs.readFileSync(legacyFile, 'utf8'));
      store.importLegacy(legacy);
      let archived = `${legacyFile}.migrated`;
      if (fs.existsSync(archived)) archived = `${archived}-${Date.now()}`;
      fs.renameSync(legacyFile, archived);
    }
  }
  ...
  switch (command) {
    case 'mcp':
      if (positional.length !== 1 || Object.keys(options).length) throw new Error('mcp accepts only the global --db option');
