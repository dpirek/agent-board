...
const tools = [
  {
    ...
    description: 'Return the authenticated user, workspace selected in Agent Board, and current UI screen state for this chat request.',
...
async function callTool(service, name, args, context = {}) {
  switch (name) {
    case 'get_current_ui_context': return result(context.uiContext || {
      available: false,
      user: null,
      workspace: null,
      screen: null,
      sourceScreen: null,
      message: 'UI context is only available to requests initiated by the authenticated Agent Board chat UI.'
    });
...
async function handleMessage(service, message, context = {}) {
  ...
  switch (message.method) {
    case 'initialize':
      return {
        ...
        instructions: 'In Agent Board chat, call get_current_ui_context first to learn the authenticated user, selected workspace, and UI screen. Use setup_workspace, create_project, and the issue/board/sprint tools for Jira-like project management. Generic record tools expose every table in db/schema.sql for advanced operations.'
      };
    case 'ping': return {};
    ...
    case 'tools/call': {
      ...
      try {
        return await callTool(service, params.name, params.arguments || {}, context);
      } catch (error) {
